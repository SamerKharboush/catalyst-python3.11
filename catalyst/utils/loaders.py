...
def get_loaders_from_params(
    ...
    **data_params,
    ...
    assert isinstance(loaders_params, dict), f"`loaders_params` should be a Dict. " f"Got: {loaders_params}"
    samplers_params = samplers_params or {}
    assert isinstance(samplers_params, dict), f"`samplers_params` should be a Dict. Got: {samplers_params}"
    ...
    for name, datasource in datasets.items():  # noqa: WPS426
        assert isinstance(datasource, (Dataset, dict)), f"{datasource} should be Dataset or Dict. Got: {datasource}"
